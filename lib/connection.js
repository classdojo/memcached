...
issues.log = function log(message) {
  ...
  if (this.failures && this.failures == this.config.failures)
    this.failuresResetId = issue.setTimeout(issue.failuresReset.bind(issue), this.failuresTimeout);

  if (this.failures && !this.locked) {
    this.locked = true;
    issue.setTimeout(issue.attemptRetry.bind(issue), this.retry);
    return this.emit("issue", this.details);
  }

  if (this.failuresResetId) issue.clearTimeout(this.failuresResetId);

  if (this.remove) return this.emit("remove", this.details);

  if (!this.isScheduledToReconnect) {
    this.isScheduledToReconnect = true;
    issue.setTimeout(issue.attemptReconnect.bind(issue), this.reconnect);
  }
};
...
issues.attemptReconnect = function attemptReconnect() {
  ...
  ping(this.tokens[1], function pingpong(err) {
    ...
    if (err) {
      ...
      return issue.setTimeout(issue.attemptReconnect.bind(issue), issue.reconnect);
    }
...
issues.setTimeout = function (fn, ms) {
  var issue = this;

  var timer = setTimeout(function setTimeoutWrappedFn () {
    issue.clearTimeout(timer);
...
issues.clearTimeout = function (timer) {
  clearTimeout(timer);
